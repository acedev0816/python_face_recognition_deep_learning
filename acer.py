...
def save_person(face,index):
    pc = photo_count[index]
    if pc == 3: 
        return
    photo_count[index] = pc + 1

    directpath = ".\\output\\Person" + str(index)
    if not os.path.exists(directpath):
        os.makedirs(directpath)
    savePath = directpath+"\\"+"image" + str(pc+1) +".jpg"
    cv2.imwrite( savePath,cv2.resize(face,(200,200)))

print("Classifying faces...")
def isVideo(path):
	lists = path.split(".")
	if lists[-1] == 'avi' or lists[-1] == 'mp4':
		return True
	return False
...
def func(path):
    ...
    while True:
        ...
        if isVideo(path):
        	faces = faceCascade.detectMultiScale(gray, scaleFactor=1.2, minNeighbors=10,minSize=(60,60)) #1.3 3
        else:
        	faces = faceCascade.detectMultiScale(gray, scaleFactor=1.03, minNeighbors=5,minSize=(60,60)) #1.3 3
        ...
        for (x,y,w,h) in faces:
            ...
            index = 0
            #first predict who is him
            if nPerson != 0:
                index,confidence,img = predict(face)
                print("person", index, "  confidience", confidence)
                if confidence > hold: #another person
                    nPerson = nPerson + 1
                ...
                index = 1
                photo_count.append(0)
            save_person(frame[y:y+h,x:x+w],index)

            face_recognizer.update([face], np.array([index]) )
